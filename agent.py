...
# Groq's OpenAI-compatible endpoint. We route through the "openai/" prefix
# (CrewAI's native OpenAI-compatible code path) instead of "groq/" because,
# as of CrewAI 1.x, the LiteLLM-based "groq/" route has a known bug with
# some Groq model names. Using base_url + the "openai/" prefix avoids it
# completely while still calling Groq's servers.
